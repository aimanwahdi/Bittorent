package bittorensimag.ProgressBar.wrapped;

import bittorensimag.ProgressBar.ProgressBarBuilder;
...
public class ProgressBarWrappedIterable<T> implements Iterable<T> {
    ...
    public Iterator<T> iterator() {
        ...
        return new ProgressBarWrappedIterator<>(it,
                pbb.setInitialMax(underlying.spliterator().getExactSizeIfKnown()).build()
        // getExactSizeIfKnown return -1 if not known, then indefinite progress bar
        // naturally
        );
    }
}
