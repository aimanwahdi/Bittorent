...
import bittorensimag.ProgressBar.wrapped.ProgressBarWrappedInputStream;
import bittorensimag.ProgressBar.wrapped.ProgressBarWrappedIterable;
import bittorensimag.ProgressBar.wrapped.ProgressBarWrappedIterator;
import bittorensimag.ProgressBar.wrapped.ProgressBarWrappedSpliterator;
...
import static bittorensimag.ProgressBar.Util.createConsoleConsumer;
...
public class ProgressBar implements AutoCloseable {
    ...
    /**
     * Creates a progress bar with the specific taskName name and initial maximum
     * value.
     * 
    ...
    /**
     * Creates a progress bar with the specific taskName name, initial maximum
     * value, customized update interval (default 1000 ms), the PrintStream to be
     * used, and output style.
     * 
     * @param task                 Task name
     * @param initialMax           Initial maximum value
     * @param updateIntervalMillis Update interval (default value 1000 ms)
     * @param style                Output style (default value
    ...
    public ProgressBar(String task, long initialMax, int updateIntervalMillis, PrintStream os, ProgressBarStyle style,
            String unitName, long unitSize, boolean showSpeed, DecimalFormat speedFormat, ChronoUnit speedUnit,
            long processed, Duration elapsed) {
     ...
     */
    public ProgressBar(String task, long initialMax, int updateIntervalMillis, long processed, Duration elapsed,
            ProgressBarRenderer renderer, ProgressBarConsumer consumer) {
        ...
        scheduledTask = (ScheduledFuture<Void>) Util.executor.scheduleAtFixedRate(action, 0, updateIntervalMillis,
    ...
    /**
     * Wraps an iterator so that when iterated, a progress bar is shown to track the
     * traversal progress.
     * 
    ...
    public static <T> Iterator<T> wrap(Iterator<T> it, String task) {
        return wrap(it, new ProgressBarBuilder().setTaskName(task).setInitialMax(-1)); // indefinite progress bar
    }
    ...
    /**
     * Wraps an {@link Iterable} so that when iterated, a progress bar is shown to
     * track the traversal progress.
     * <p>
    ...
    /**
     * Wraps an {@link Iterable} so that when iterated, a progress bar is shown to
     * track the traversal progress. For this function the progress bar can be fully
     * customized by using a {@link ProgressBarBuilder}.
     * 
     * @param ts  Underlying iterable
    ...
    /**
     * Wraps an {@link InputStream} so that when read, a progress bar is shown to
    ...
    /**
     * Wraps an {@link InputStream} so that when read, a progress bar is shown to
     * track the reading progress. For this function the progress bar can be fully
     * customized by using a {@link ProgressBarBuilder}.
     * 
     * @param is  Input stream to be wrapped
     * @param pbb An instance of a {@link ProgressBarBuilder}
    ...
    /**
     * Wraps a {@link Spliterator} so that when iterated, a progress bar is shown to
     * track the traversal progress.
     * 
    ...
    /**
     * Wraps a {@link Stream} so that when iterated, a progress bar is shown to
     * track the traversal progress.
     * 
    ...
    /**
     * Wraps an array so that when iterated, a progress bar is shown to track the
     * traversal progress.
     * 
    ...
    /**
     * Wraps an array so that when iterated, a progress bar is shown to track the
     * traversal progress. For this function the progress bar can be fully
     * customized by using a {@link ProgressBarBuilder}.
     * 
     * @param array Array to be wrapped
     * @param pbb   An instance of a {@link ProgressBarBuilder}
